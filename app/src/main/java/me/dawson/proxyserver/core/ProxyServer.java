...
public class ProxyServer {
	...
	private static final int DEFAULT_PORT = 6821;
	private static final int MAX_PORT = 50146; // real max can be 65535
	...
	public synchronized boolean start() {
        ...
        try {
            /*
             * Bind the proxy server to an IPv6-capable socket.  When the
             * underlying platform supports it, we attempt to open an IPv6
             * server socket and explicitly disable the IPv6-only restriction
             * so that both IPv4 and IPv6 clients can connect.  This change
             * enables native dual-stack forwarding rather than relying on
             * implicit address family mapping.  If opening an IPv6 socket
             * fails (for example on older Android versions), we fall back to
             * the default implementation.
             */
            try {
                server = ServerSocketChannel.open();
                // disable IPV6-only mode so IPv4 clients are also accepted
                //server.setOption(StandardSocketOptions.IPV6_ONLY, false);
            } catch (Exception ex) {
                // fallback to default if IPv6 sockets are unsupported
                server = ServerSocketChannel.open();
            }
