...
import static com.penguinehis.ultrasshservice.tunnel.TunnelUtils.getnewserver;
...

{
	...
	public SSLTunnelProxy(String server, int port, String hostSni, Context context) {
	...
	public Socket openConnection(String hostname, int port, int connectTimeout, int readTimeout) throws IOException
	{
		...
		// Ensure the TLS tunnel socket bypasses the VPN to prevent routing loops.
		SocketProtector.protect(mSocket);
		mSocket.connect(new InetSocketAddress(getnewserver(stunnelServer,1), stunnelPort));

		if (mSocket.isConnected()) {
			mSocket = doSSLHandshake(hostname, getnewsni(stunnelHostSNI, 1), port);
		}
