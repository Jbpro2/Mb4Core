...
import static com.penguinehis.ultrasshservice.tunnel.TunnelUtils.getnetHostname;
import static com.penguinehis.ultrasshservice.tunnel.TunnelUtils.getnewsni;

import android.content.Context;
import android.util.Log;

import com.penguinehis.ultrasshservice.BuildConfig;
...
public class SSL_PROXY_TUNNEL implements ProxyData
{
	...
	public Socket openConnection(String hostname, int port, int connectTimeout, int readTimeout) throws IOException
	{
		...
		String proxy = TunnelUtils.getNextProxy(this.stunnelServer,1);
		// Ensure the upstream socket bypasses the VPN to prevent routing loops.
		SocketProtector.protect(mSocket);
		mSocket.connect(new InetSocketAddress(proxy, stunnelPort));
		//Log.d("SSL", hostname+ ""+ port + "" + stunnelHostSNI);
		if (mSocket.isConnected()) {
			mSocket = doSSLHandshake(getnetHostname(hostname,1), getnewsni(stunnelHostSNI, 1), stunnelPort);
		}
	...
	private SSLSocket doSSLHandshake(String host, String sni, int port) throws IOException {
		...
		try {
			...
			TLSSocketFactory tsf = new TLSSocketFactory(mContext);
