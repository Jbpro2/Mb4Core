package com.penguinehis.ultrasshservice;
...
import com.penguinehis.ultrasshservice.aidl.IUltraSSHServiceInternal;
...
import com.penguinehis.ultrasshservice.tunnel.TunnelManagerThread;
...
import com.penguinehis.ultrasshservice.util.DummyActivity;

public class SocksReviveService extends Service
implements SkStatus.StateListener {
	private static final String TAG = SocksReviveService.class.getSimpleName();
	public static final String START_SERVICE = "com.penguinehis.socksrevive:startTunnel";
	...
	private final IBinder mBinder = new IUltraSSHServiceInternal.Stub() {
		...
		@Override
		public void stopVPN() {
			SocksReviveService.this.stopTunnel();
		}
	...
	public void onCreate() {
		...
		super.onCreate();
		wakeLock = ((PowerManager)getSystemService(Context.POWER_SERVICE)).newWakeLock(1, "RSocketClay::WakelockService");
	...
	public synchronized void stopTunnel() {
		...
		if(mPrefs.getWakelock()){
			wakeLock.release();
			SkStatus.logInfo("CPU Wakelock Deactivated");
	...
	public void endTunnelService() {
		mHandler.post(new Runnable() {
			...
			public void run() {
				...
				stopSelf();
				SkStatus.removeStateListener(SocksReviveService.this);
			}
		});
	}
...
//		PendingIntent reconnectPendingIntent = PendingIntent.getBroadcast(this, 0, reconnectVPN, PendingIntent.FLAG_CANCEL_CURRENT | PendingIntent.FLAG_MUTABLE);
//
...
//				getString(R.string.reconnect), reconnectPendingIntent);
		...
		PendingIntent disconnectPendingIntent = PendingIntent.getBroadcast(this, 0, disconnectVPN, PendingIntent.FLAG_CANCEL_CURRENT | PendingIntent.FLAG_MUTABLE);
	...
	public static PendingIntent getGraphPendingIntent(Context context) {
		...
		intent.setComponent(new ComponentName(context, context.getPackageName() + ".SocksReviveMainActivity"));
		intent.addFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT);

		PendingIntent startLW = PendingIntent.getActivity(context, 0, intent,  PendingIntent.FLAG_MUTABLE);
	...
	public static final String TUNNEL_SSH_RESTART_SERVICE = SocksReviveService.class.getName() + "::restartservicebroadcast",
			TUNNEL_SSH_STOP_SERVICE = SocksReviveService.class.getName() + "::stopservicebroadcast";
	...
	private void stopdnstt(){
		final SharedPreferences prefs = mConfig.getPrefsPrivate();
		final String servermodecheck = prefs.getString(Settings.SERVIDOR_KEY, "sshServer");

		if (servermodecheck.toLowerCase().contains("127.0.0.1")){

			try{
				Intent intent_stopdnstt = new Intent();
				intent_stopdnstt.putExtra("intent_stop_dnstt", "true");
				intent_stopdnstt.setComponent(new ComponentName("socksrevive.plugin.dragon2", "socksrevive.plugin.dragon2.DNSTTService"));

				if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
					SocksReviveService.this.startForegroundService(intent_stopdnstt);
				} else {
					startService(intent_stopdnstt);
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
